...
const detailProduct = () => __awaiter(void 0, void 0, void 0, function* () {
    const response = yield fetch(`http://localhost:3000/API/product/${id_pro}`);
    const images = yield fetch(`http://localhost:3000/API/product/images/${id_pro}`);
    const pro = yield response.json();
    const imgpro = yield images.json();
    console.log(imgpro);
    const detailProduct = document.querySelector('.motasp');
    let dshinhnho = '';
    let dshinhlon = '';
    imgpro.forEach((img, i) => {
        dshinhnho +=
            `
